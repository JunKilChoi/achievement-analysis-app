...
1. 전체 평가 결과 진단
- 평균, 표준편차, 최고점, 최저점, 성취수준 분포를 바탕으로 시험의 전반적 난이도를 진단하라.
- 점수 분포가 상위 집중형, 하위 집중형, 중간 집중형, 양극화형 중 어디에 가까운지 해석하라.
- 수치 나열에 그치지 말고, 이 분포가 수업 및 평가 측면에서 어떤 의미를 갖는지 설명하라.

2. 학급별 결과 비교
- 학급별 평균, 표준편차, 최고점, 최저점, 성취수준 분포를 비교하라.
- 전체 경향과 다른 학급이 있으면 통계적으로 확인되는 차이를 근거로 해석하라.
- 단순히 높고 낮음을 말하지 말고, 분포의 폭과 성취수준 구성까지 함께 해석하라.

3. 문항별 통계 진단
- 정답률이 낮은 문항, 정답률이 높은 문항, 변별도가 낮은 문항, 변별도가 높은 문항을 분류하라.
- 예상 난이도와 실제 정답률이 다른 문항을 정리하라.
- 문항 내용은 보지 않았으므로, 원인을 단정하지 말고 원안지에서 점검해야 할 가능성으로 제시하라.

4. 선택지 반응 경향
- 특정 오답 선택지에 응답이 몰린 문항을 찾아라.
- 선택지 내용은 확인할 수 없으므로, 특정 오개념을 단정하지 말고 학생 반응이 집중된 문항으로 표시하라.
- 원안지 기반 고급 분석에서 우선 확인할 문항으로 제안하라.

5. 평가영역 및 성취기준별 보충 우선순위
- 평가영역별, 성취기준별 정답률을 바탕으로 강점 영역과 취약 영역을 구분하라.
- 보충 지도가 필요한 성취기준의 우선순위를 제시하라.
- 각 성취기준에 대해 보완해야 할 사고 과정이나 개념을 통계 자료에 근거하여 추론하라.

6. 성취수준별 집단 특성
- A~E 수준별 정답률 차이가 큰 문항을 중심으로 상위권과 하위권을 가르는 문항을 찾으라.
- 상위권도 어려워한 문항과 하위권에게 특히 부담이 컸던 문항을 구분하라.
- A/B, C, D/E 집단별 피드백 방향을 각각 제안하라.

7. 고급 분석 필요 문항 추천
- 원안지 기반 심층 분석이 필요한 문항을 5~10개 추천하라.
- 추천 사유를 정답률, 변별도, 선택지 반응률, 난이도 괴리, 성취수준별 격차 중 하나 이상으로 설명하라.
- 추천 문항은 원안지 확인 시 어떤 지점을 우선 봐야 하는지도 함께 제시하라.

8. 수업 개선 및 피드백 방향
- 다음 수업에서 우선 보완할 내용 3가지를 제시하라.
- 학생 피드백 방향을 전체 피드백, 수준별 피드백, 개별 보충 피드백으로 나누어 제안하라.
- 다음 평가 설계에서 점검할 부분을 제안하라.

작성 원칙:
- 수치를 근거로 하되, 가능한 해석을 풍성하게 제시하라.
- 통계로 확인되는 사실과 가능성으로 제시해야 할 해석을 구분하라.
- 문항 내용을 확인할 수 없다는 말만 반복하지 말고, 현재 주어진 통계 자료로 가능한 해석을 최대한 끌어내라.
- 문항 표현, 자료 구성, 선지 유인, 구체적 오개념은 원안지 없이 단정하지 말고 고급 분석에서 확인할 점으로 제시하라.
