...
- v1.112: 문항별 분석의 예상 난이도-실제 정답률 일치 여부에 도움말 툴팁 추가
- v1.113: 학급별 분석의 학급별 문항 분석과 학급 간 정답률 차이가 큰 문항에 도움말 툴팁 추가
- v1.114: 평가영역별 분석에 해석 중심 도움말 툴팁 추가
...
def main() -> None:
    ...
    if selected_analysis_tab == "데이터 확인":
        ...
        st.subheader(
            "예상 난이도-실제 정답률 일치 여부",
            help="문항정보표에 입력한 예상 난이도와 실제 학생 정답률이 잘 맞았는지 비교하는 영역입니다. 설정한 정답률 기준에 따라 실제 난이도를 판단하고, 예상보다 어려웠던 문항이나 예상보다 쉬웠던 문항을 확인할 수 있습니다. 문항의 난이도 설정이 적절했는지, 학생들이 예상과 다르게 어려워한 문항은 무엇인지 살펴보세요.",
        )
        ...
        with st.container(border=True):
            st.subheader(
                "학급별 문항 분석",
                help="학급별로 각 문항의 정답률을 비교하는 표입니다. 특정 학급에서 유독 정답률이 낮은 문항은 그 반에서 해당 개념이나 자료 해석 과정이 충분히 정리되지 않았을 가능성을 보여줍니다. 전체 정답률뿐 아니라 반별 차이를 함께 보며, 학급별 보충 설명이나 피드백이 필요한 문항을 확인하세요.",
            )
            st.markdown(
        ...
        with st.container(border=True):
            st.subheader(
                "학급 간 정답률 차이가 큰 문항",
                help="학급 간 정답률 차이가 크게 나타난 문항을 따로 보여주는 표입니다. 같은 문항인데도 반별 결과 차이가 크다면 수업 진행 방식, 활동 경험, 개념 정리 정도, 문항을 받아들이는 방식에 차이가 있었는지 점검할 수 있습니다. 특정 반만 낮은 문항은 해당 반 중심으로, 여러 반에서 낮은 문항은 전체 보충 지도로 연결해 보세요.",
            )
        ...
        with st.container(border=True):
            st.subheader(
                "평가영역별 분석",
                help="평가영역별로 학생들의 성취 정도를 비교하는 영역입니다. 특정 영역의 결과가 낮게 나타났다면 단순히 점수가 낮다는 의미를 넘어, 그 영역에서 요구한 개념 이해, 자료 해석, 계산 과정, 실험 설계, 추론 등의 사고 과정 중 어디에서 어려움이 있었는지 살펴볼 필요가 있습니다. 학생의 사고가 막힌 지점을 더 자세히 파악하려면 이후 AI 분석 기능을 활용하여 평가영역별 결과를 심층적으로 해석해 보세요.",
            )
            st.info(
        ...
        with st.container(border=True):
            st.subheader(
                "성취기준별 분석",
                help="성취기준별로 학생들의 도달 정도를 확인하는 영역입니다. 하나의 성취기준은 개념 이해, 자료 해석, 원리 적용, 탐구 과정 등 여러 평가요소를 포함할 수 있으므로, 결과가 낮게 나타났다고 해서 단순히 성취기준 전체를 달성하지 못했다고 해석하기보다 어떤 평가요소에서 어려움이 있었는지 함께 살펴볼 필요가 있습니다. 관련 문항과 평가요소를 함께 확인하면 성취기준 도달 여부를 더 구체적으로 고찰할 수 있으며, 이후 AI 분석 기능을 활용해 학생의 이해가 막힌 지점과 보충 지도가 필요한 부분을 더 자세히 해석할 수 있습니다.",
            )
            standard_cols = [c for c in ["성취기준", "평가영역", "문항번호", "문항수", "배점합계", "평균점수", "정답률"] if c in analysis["standard"].columns]
        ...
        with st.container(border=True):
            st.subheader(
                "성취수준별 문항 분석",
                help="성취수준별로 학생들이 각 문항을 얼마나 맞혔는지 확인하는 영역입니다. 표의 값은 해당 성취수준에 속한 학생들 중 그 문항을 맞힌 학생의 비율을 의미합니다. 예를 들어 A 수준의 1번 문항 정답률이 90%라면, A 수준 학생의 90%가 1번 문항을 해결했다는 뜻입니다. 이 표는 단순히 문항별 정답률을 보는 것보다, 성취수준에 따라 학생들의 이해가 어떻게 달라지는지 해석하는 데 활용할 수 있습니다. 상위 성취수준에서도 정답률이 낮은 문항은 전체적으로 어려웠거나 문항에서 요구한 사고 과정이 복잡했을 가능성이 있고, 특정 성취수준부터 정답률이 급격히 낮아지는 문항은 그 지점에서 학생들의 이해가 갈라졌을 가능성을 보여줍니다. 성취수준별 차이가 큰 문항을 중심으로 어떤 개념이나 풀이 과정에서 차이가 생겼는지 살펴보세요.",
            )
            st.info(
